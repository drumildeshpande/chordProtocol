...
case class stabilize(actNodeArray:Array[Int])
...
class MasterActor(ActiveNodes: Int, NumMsgs: Int, Actsys: ActorSystem) extends Actor {
  ...
  val failure = context.system.scheduler.scheduleOnce(5 seconds,self,"failNode")
  ...
  def receive = {
    ...
    case "failNode" =>{
    //This method is called at some point of time to fail some random node
    // The system then handles this node failure dynamically
        println("Ola failure ...")
        val rIndex = Random.nextInt(ActiveNodes-1)
        var failedNode = context.actorSelection("akka://ChordProtocol/user/node_"+ActNodeArray(rIndex).toString)
        failedNode ! PoisonPill
      }
...
class WorkerActor(ActiveNodes:Int, totalnodes:Int , NumMsgs:Int ,Nid:Int, Master:ActorRef) extends Actor{
  ...
  var hopsCount:Int=0
  var actNodes:Array[Int]= Array(0)
  ...
  case stabilize(actNodeArray:Array[Int])=>{
      actNodes = actNodeArray
      self ! updateSucc(actNodes,actNodes.length)
    } 
  }
  ...
  val isAlive = context.actorSelection("akka://ChordProtocol/user/node_"+nextNode.toString()).resolveOne(0.1 seconds).value    
    if(!isAlive.isDefined)
    {
      //println("ttttttttttttttttttttttttttttttttttttttttttttttttttttt")
      val nextHopNode = context.actorSelection("akka://ChordProtocol/user/node_"+nextNode.toString())    
      nextHopNode ! passMsg(randNode,hopCnt+1)
    }
    else
    {
      //println("Detected node death in node: "+Nid+", for node: "+nextNode)
      var j:Int = -1
      for(i<-0 to actNodes.length-1)
      {
        if(actNodes(i)==nextNode)
         j=i;
      }
      if(j>=0)
      {
        val temp = actNodes.toBuffer
        temp.remove(j)
        actNodes = temp.toArray
        self ! updateSucc(actNodes,actNodes.length)
        
        var list = actNodes.toBuffer
        list.toList
        list.distinct
        for(i<-0 to list.length-1)
        {
          val notifyNode = context.actorSelection("akka://ChordProtocol/user/node_"+list(i).toString())    
          notifyNode ! stabilize(actNodes)
        }
        self ! intervalSearch(randNode,hopCnt)
      }
    }
  }
}
